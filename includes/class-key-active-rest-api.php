...
class Key_Active_REST_API {
    ...
    public function register_routes() {
        register_rest_route(
            ...
            array(
                'methods'             => 'GET',
                'callback'            => array( $this, 'check_key_active' ),
    ...
    public function check_key_active(WP_REST_Request $request)
    {
        ...
        if ( $post ) {
            ...
            if ( $status !== 'true' ) {
                 $data = [
                    "error"   => 104, // Sửa mã lỗi trùng lặp
                    "success" => false,
                    "msg"     => "Bị vô hiệu hoá! Liên hệ Zalo: <strong style='color:green'> 0813.908.901 </strong>.",
                ];
                return rest_ensure_response( $data );
            }
            ...
            if ( !$isExpire && time() > strtotime($expire) ) {
                $data = [
                    "error"   => 105,
                    "success" => false,
