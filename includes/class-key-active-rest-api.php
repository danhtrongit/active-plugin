...
class Key_Active_REST_API {
    ...
    public function check_key( WP_REST_Request $request ) {
        ...
        if ( ! $post_id ) {
            return new WP_Error( 'invalid_license', "License: {$license_key} - không hợp lệ.", array( 'status' => 404, 'error' => 103 ) );
        }
        ...
        if ( $status !== 'true' ) {
            return new WP_Error( 'license_disabled', 'License đã bị vô hiệu hóa. Liên hệ Zalo: 0813.908.901.', array( 'status' => 403, 'error' => 104 ) );
        }
        ...
        if ( $is_expired ) {
            return new WP_Error( 'license_expired', 'License đã hết hạn.', array( 'status' => 403, 'error' => 105 ) );
        }
